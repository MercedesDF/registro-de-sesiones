...
// Colores del Tema Azul (usados en el Modal) y clases de Tailwind correspondientes
const ACCENT_COLOR_CLASS = 'bg-blue-500'; // Corresponde a ACCENT_COLOR #3B82F6
const ACCENT_HOVER_COLOR_CLASS = 'hover:bg-blue-400'; // Corresponde a ACCENT_HOVER_COLOR #60A5FA
...
const TAB_INACTIVE_BG_HOVER_COLOR_CLASS = 'hover:bg-blue-50'; // Corresponde a TAB_INACTIVE_BG_HOVER_COLOR #EFF6FF
// Para TAB_ACTIVE_BG_COLOR y ACCENT_COLOR en texto/borde de pestaña, se usan clases directas de Tailwind o valores arbitrarios
const TAB_ACTIVE_TEXT_BORDER_COLOR_CLASS = 'text-blue-500 border-blue-500';
const TAB_ACTIVE_BG_COLOR_CLASS = 'bg-blue-50'; // Un azul muy claro, o SURFACE_MODAL_COLOR si se prefiere
...
const ProjectManagementModal: React.FC<ProjectManagementModalProps> = ({
  ...
  return (
    ...
    >
      ...
      >
        {/* Cabecera del Modal */}
        <div style={{ borderColor: BORDER_PRIMARY_COLOR }} className="flex justify-between items-center p-4 sm:p-6 border-b">
          <h2 id="modal-title" style={{ color: TEXT_PRIMARY_COLOR }} className={`text-xl sm:text-2xl font-semibold text-blue-600`}> {/* Usar clase Tailwind para color de acento */}
            Historial y Gestión de Proyectos
          </h2>
          <button
            onClick={onClose}
            style={{ color: TEXT_SECONDARY_COLOR }}
            className="hover:text-gray-700 transition-colors p-1 -m-1" // Ajuste de padding para área de click más grande
        ...
        <div style={{ borderColor: BORDER_PRIMARY_COLOR }} className="flex border-b">
          <button
            ...
            className={`flex-1 py-3 px-4 text-sm sm:text-base font-medium transition-colors ${activeTab === 'sessions' ? `border-b-2 ${TAB_ACTIVE_TEXT_BORDER_COLOR_CLASS} ${TAB_ACTIVE_BG_COLOR_CLASS}` : `text-gray-600 ${TAB_INACTIVE_BG_HOVER_COLOR_CLASS} hover:text-blue-600`}`}
          ...
          <button
            ...
            className={`flex-1 py-3 px-4 text-sm sm:text-base font-medium transition-colors ${activeTab === 'projects' ? `border-b-2 ${TAB_ACTIVE_TEXT_BORDER_COLOR_CLASS} ${TAB_ACTIVE_BG_COLOR_CLASS}` : `text-gray-600 ${TAB_INACTIVE_BG_HOVER_COLOR_CLASS} hover:text-blue-600`}`}
        ...
        <div className="p-4 sm:p-6 overflow-y-auto flex-grow" style={{color: TEXT_PRIMARY_COLOR}}>
                  ...
                  <table className="min-w-full divide-y border-gray-300"> {/* Estilo de borde directo en tabla */}
                    <thead className="bg-gray-100"> {/* Estilo de fondo directo en thead */}
                      <tr>
                        <th className="px-2 py-3 sm:px-4 text-left text-xs font-medium uppercase tracking-wider text-gray-700"> {/* Color de texto explícito */}
                          <input 
                            type="checkbox"
                            className="form-checkbox h-4 w-4 sm:h-5 sm:w-5 border-gray-400 rounded focus:ring-offset-0 text-blue-600 focus:ring-blue-500" // Clases Tailwind para estilo y foco. accentColor no es necesario si se usa text-blue-600
                    ...
                    <tbody className="bg-white divide-y border-gray-300">
                      {sortedSessions.map(session => (
                        <tr key={session.id} className={`transition-colors ${selectedSessionIds.has(session.id) ? `bg-blue-100` : `hover:bg-gray-50`}`}>
                          <td className="px-2 py-3 sm:px-4 whitespace-nowrap text-xs sm:text-sm">
                            <input
                              type="checkbox"
                              className="form-checkbox h-4 w-4 sm:h-5 sm:w-5 border-gray-400 rounded focus:ring-offset-0 text-blue-600 focus:ring-blue-500"
                          ...
                          <td className="px-2 py-3 sm:px-4 whitespace-nowrap text-xs sm:text-sm">
                            <select
                              ...
                              className="w-full border-gray-400 text-gray-800 bg-white text-xs sm:text-sm rounded-md p-1.5 focus:ring-blue-500 focus:border-blue-500"
          ...
          {activeTab === 'projects' && (
            <div>
              <form onSubmit={handleAddProjectSubmit} className="mb-6 flex gap-2 sm:gap-4">
                <input
                  type="text"
                  value={newProjectName}
                  onChange={(e) => setNewProjectName(e.target.value)}
                  placeholder="Nombre del nuevo proyecto"
                  className="flex-grow border-gray-400 text-gray-800 bg-white placeholder-gray-400 placeholder-opacity-75 text-sm sm:text-base rounded-md p-2.5 focus:ring-blue-500 focus:border-blue-500"
                ...
                <ul className="space-y-2">
                   ...
                   <li className="bg-gray-100 text-gray-700 border-b border-gray-300 p-3 sm:p-4 rounded-t-md flex items-center justify-between text-xs font-medium uppercase tracking-wider">
                      <div className="flex items-center flex-grow min-w-0"> {/* min-w-0 para truncar texto si es necesario */}
                        <input 
                          type="checkbox"
                          className="form-checkbox h-4 w-4 sm:h-5 sm:w-5 border-gray-400 rounded focus:ring-offset-0 text-blue-600 focus:ring-blue-500 mr-2 sm:mr-3 flex-shrink-0"
                  ...
                  {sortedProjects.map(project => (
                    <li 
                      ...
                      className={`p-3 sm:p-4 flex items-center justify-between transition-colors border rounded-b-md
                        ...
                        ${selectedProjectIds.has(project.id) ? 'bg-blue-100 border-blue-500' : 'bg-white border-gray-300'}`} // Clases Tailwind para selección
                    >
